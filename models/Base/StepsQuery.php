 ...
 * @method     ChildStepsQuery orderByRecipeid($order = Criteria::ASC) Order by the recipeid column
 *
 ...
 * @method     ChildStepsQuery groupByRecipeid() Group by the recipeid column
 *
 ...
 * @method     ChildSteps findOneByRecipeid(int $recipeid) Return the first ChildSteps filtered by the recipeid column *
 ...
 * @method     ChildSteps requireOneByRecipeid(int $recipeid) Return the first ChildSteps filtered by the recipeid column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildSteps[]|ObjectCollection findByRecipeid(int $recipeid) Return ChildSteps objects filtered by the recipeid column
...
abstract class StepsQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT id, step_number, description, recipeid FROM steps WHERE id = :p0';
        try {
    ...
    /**
     * Filter the query on the recipeid column
     *
     * Example usage:
     * <code>
     * $query->filterByRecipeid(1234); // WHERE recipeid = 1234
     * $query->filterByRecipeid(array(12, 34)); // WHERE recipeid IN (12, 34)
     * $query->filterByRecipeid(array('min' => 12)); // WHERE recipeid > 12
     * </code>
     *
    ...
    public function filterByRecipeid($recipeid = null, $comparison = null)
    {
        if (is_array($recipeid)) {
            ...
            if (isset($recipeid['min'])) {
                $this->addUsingAlias(StepsTableMap::COL_RECIPEID, $recipeid['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($recipeid['max'])) {
                $this->addUsingAlias(StepsTableMap::COL_RECIPEID, $recipeid['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(StepsTableMap::COL_RECIPEID, $recipeid, $comparison);
    }
